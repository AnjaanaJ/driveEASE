...
const statusStyles = {
  Approved: {
    ...
    detail:
      "Your registration is approved. Your attendance and documents are available below.",
  },
  Rejected: {
  ...
  Pending: {
    ...
    detail:
      "Your registration is awaiting admin approval. Attendance and document upload will unlock after approval.",
  },
};
...
function StudentProfilePage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background p-8 text-text-secondary">
        Loading profile...
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-8">
          ...
          <p className="mt-2 text-text-secondary">
            Your personal details, learning preferences, and records in one
            place.
          </p>
        </div>
        {(success || error) && (
          <div
            className={`mb-6 rounded-2xl border px-4 py-3 text-sm ${success ? "border-emerald-400/25 bg-emerald-500/10 text-emerald-200" : "border-red-400/25 bg-red-500/10 text-red-200"}`}
          >
            {success || error}
          </div>
        )}
        ...
        <div className="space-y-6">
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
              ...
              {!editing && (
                <button
                  ...
                  className="rounded-xl bg-gradient-to-r from-primary to-secondary px-5 py-2.5 text-sm font-semibold text-white transition hover:opacity-90"
                >
                  Edit profile
                </button>
              )}
            ...
            {editing ? (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full rounded-xl border border-white/10 bg-background/70 px-4 py-3 text-text-primary outline-none transition placeholder:text-text-secondary focus:border-accent focus:ring-2 focus:ring-accent/20"
                  />
                </div>
                <div>
                  <label
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows="3"
                    className="w-full resize-none rounded-xl border border-white/10 bg-background/70 px-4 py-3 text-text-primary outline-none transition placeholder:text-text-secondary focus:border-accent focus:ring-2 focus:ring-accent/20"
                  />
                </div>
                <div>
                  <label
                    htmlFor="preferredVehicleType"
                    className="mb-2 block text-sm font-medium text-text-secondary"
                  >
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    required
                    className="w-full rounded-xl border border-white/10 bg-background/70 px-4 py-3 text-text-primary outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/20"
                  >
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="rounded-xl border border-white/10 px-5 py-2.5 text-sm font-medium text-text-primary transition hover:bg-white/5"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <dl className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  <DetailItem
                    label="Address"
                    value={student.address || "Not added yet"}
                  />
                </div>
              </dl>
            )}
          ...
          <section className="relative overflow-hidden rounded-3xl border border-white/20 bg-white/[0.03] backdrop-blur-3xl p-6 shadow-[0_8px_32px_rgba(0,0,0,0.4)] sm:p-8">
            ...
            <p className="mt-1 text-sm text-text-secondary">
              Your selected package and learning setup.
            </p>
            <div className="mt-6 space-y-4">
              <OverviewItem
                label="Course package"
                value={student.coursePackage?.name || "Not assigned yet"}
              />
              <OverviewItem
                label="Preferred vehicle"
                value={
                  student.preferredVehicleType
                    ? `${student.preferredVehicleType}${student.preferredTransmission ? ` · ${student.preferredTransmission}` : ""}`
                    : "Not selected yet"
                }
              />
              <OverviewItem
...
function DetailItem({ label, value, accent = false }) {
  return (
    <div className="rounded-2xl border border-white/5 bg-background/35 p-4">
      ...
      <dd
        className={`mt-2 break-words font-medium ${accent ? "text-accent" : "text-text-primary"}`}
      >
        {value}
      </dd>
    </div>
